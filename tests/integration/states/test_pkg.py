...
class PkgTest(ModuleCase, SaltReturnAssertsMixin):
    ...
    @requires_salt_state_modules('pkg.installed', 'pkg.removed')
    ...
    @requires_salt_state_modules('pkg.latest', 'pkg.removed')
    ...
    @requires_salt_state_modules('pkg.latest')
    ...
    @requires_salt_modules('pkg.hold', 'pkg.unhold', 'pkg.version')
    @requires_salt_state_modules('pkg.installed', 'pkg.removed')
    def test_pkg_015_installed_held(self):
        ...
        try:
            ...
            self.assertSaltTrueReturn(ret)
            if pre_grains and 'redhat' in pre_grains.like():
                ret = self.run_state('pkg.removed', name='yum-plugin-versionlock')
                self.assertSaltTrueReturn(ret)
    ...
    @skipIf(not _PKG_CAP_TARGETS, 'Capability not available')
    @requires_salt_modules('pkg.version')
    @requires_salt_state_modules('pkg.installed', 'pkg.removed', 'pkg.downloaded')
    ...
    @skipIf(not _PKG_CAP_TARGETS, 'Capability not available')
    @requires_salt_modules('pkg.version')
    @requires_salt_state_modules('pkg.installed', 'pkg.removed', 'pkg.uptodate')
